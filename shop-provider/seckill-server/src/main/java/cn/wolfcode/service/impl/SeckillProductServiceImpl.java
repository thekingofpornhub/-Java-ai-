...
//    private RocketMQTemplate rocketMQTemplate;
    ...
    public List<SeckillProductVo> selectTodayListByTime(Integer time) {
        ...
        List<SeckillProductVo> productVoList = todayList.stream()
                .map(sp -> {
                    ...
                    BeanUtils.copyProperties(sp, vo);//秒杀id

                    // 遍历远程查询的商品列表，判断是否与当前的秒杀商品关联的商品对象一致
                    // 如果是一致的，将该对象返回并将属性拷贝到 vo 对象中
                    List<Product> list = products.stream().filter(p -> sp.getProductId().equals(p.getId())).collect(Collectors.toList());//将秒杀和商品id对应的打印出来
                    if (list.size() > 0) {
                        Product product = list.get(0);
                        BeanUtils.copyProperties(product, vo);
                    }
                    vo.setId(sp.getId());//设置成秒杀id
    ...
    public List<SeckillProductVo> selectTodayListByTimeFromRedis(Integer time) {
        ...
        if (stringList == null || stringList.size() == 0) {
            ...
            return this.selectTodayListByTime(time);//redis在另一个地方维护
        }

        return stringList.stream().map(json -> JSON.parseObject(json, SeckillProductVo.class)).collect(Collectors.toList());//将json转化为对象，返回集合
    }
